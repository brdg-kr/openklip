...
| `working/media-index.json` | Rebuildable face timelines and SigLIP2 scene vectors |
| `working/media-index-status.json` | Background media-index phase and terminal status |
...
Verified against the current codebase (`VERSION` / `package.json` `0.44.3`, 2771 tests: 2757 pass, 14 skip without `OPENKLIP_INTEGRATION=1`/`OPENKLIP_ACCEPTANCE=1` and env-gated fixtures):
...
- **Target member media index**: MCP `member_profile_create` builds a reusable YuNet and SFace identity profile from 3–40 verified positive images plus optional same-group or explicit hard-negative images; remote references stream through a 20 MB limit and reject credentials, private hosts, unsafe redirects, and unsupported image types. `url_ingest` can bind that profile and completes a 4 fps face pass, quality-filtered 10 fps boundary refinement, shot-boundary sampling, and whole-frame plus target-context plus target-medium SigLIP2 indexing before its job reports success. Member-aware `moment_search` keeps the Korean query and can fuse up to two faithful English visual variants while returning per-query scores and crop scopes. `media_index_status`, `media_index_rebuild`, `media_index_cancel`, and `member_appearance_search` expose the derived results without putting biometric vectors in `project.json` or MCP output; `target-moment-search` is the matching Agent skill
...
- **Project creation**: upload a video in the New Project dialog, **Import folder…** (largest video ingests, other media lands in `assets/`), **Import from URL** (yt-dlp on PATH), drop one or many files onto the empty workspace, or call MCP `url_ingest` and poll `ingest_job_status`; MCP URL ingest optionally binds a `memberProfileId` and starts the reusable media index after the project source is durable; URL projects use a stable YouTube video-id slug or a collision-resistant generic URL slug; uploads stream to disk with size caps (12 GB project, 4 GB asset); format-validated on client and server, source persisted into the project folder, explicit overwrite confirm on name collisions, ingest progress overlay, editor opens on completion
...
- **MCP server**: `openklip mcp` (stdio) exposes 106 agent tools plus 3 deferred-surface discovery tools across project creation, member profiling and indexing, query, mutation, task progress, revert, and export surfaces; `.cursor/mcp.json` wired for Cursor
